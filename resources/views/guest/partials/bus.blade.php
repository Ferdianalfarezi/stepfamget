...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header {
    ...
    padding: calc(16px + var(--safe-top)) 20px 28px;
  }
  ...
  .header-label { font-size:10px; font-weight:600; color:rgba(255,255,255,.4); letter-spacing:1px; }
  .header-title { font-size:17px; font-weight:800; color:#fff; margin-top:2px; }
  ...
  /* ── Plat Nomor ── */
  .plat-divider {
    border: none; border-top: 1px solid var(--border); margin: 4px 0 14px;
  }
  .plat-label {
    font-size: 12px; font-weight: 700; color: var(--text-light);
...
<div style="background:var(--green-dark);border-radius:18px;padding:18px;margin-bottom:14px;position:relative;overflow:hidden;">
  ...
  <div style="font-size:22px;font-weight:800;color:#fff;margin-bottom:4px;">Kamu mau naik apa nih nanti?</div>
...
<div class="content">
    ...
    <div class="status-detail">
      ...
      <div class="status-value" id="statusValue">
        @if($currentPilihan === 'bus')
          Naik Bus
        @elseif($currentPilihan === 'kendaraan')
          {{ ucfirst($currentJenis) }} Pribadi
        @endif
      </div>
  ...
  <div class="pilihan-grid">
    <div class="pilihan-card {{ $currentPilihan === 'bus' ? 'selected' : '' }}"
      ...
      <div class="pilihan-check" id="checkBus">
        <i class="fa-solid fa-check" style="font-size:10px;"></i>
      </div>
      <div class="pilihan-icon bus-icon">
    ...
    <div class="pilihan-card {{ $currentPilihan === 'kendaraan' ? 'selected' : '' }}"
      ...
      <div class="pilihan-check" id="checkKendaraan">
        <i class="fa-solid fa-check" style="font-size:10px;"></i>
      </div>
      <div class="pilihan-icon kend-icon">
  ...
  {{-- Detail Kendaraan: Jenis + Plat (muncul kalau pilih kendaraan) --}}
  <div class="plat-wrap {{ $currentPilihan === 'kendaraan' ? 'show' : '' }}" id="platWrap">

    {{-- Jenis Kendaraan --}}
    <div class="jenis-title">JENIS KENDARAAN</div>
    <div class="jenis-grid">
      <div class="jenis-btn {{ $currentJenis === 'mobil' ? 'selected' : '' }}"
           data-jenis="mobil" onclick="selectJenis('mobil')">
        <span class="jenis-btn-emoji">🚗</span>
        <span class="jenis-btn-label">Mobil</span>
      </div>
      <div class="jenis-btn {{ $currentJenis === 'motor' ? 'selected' : '' }}"
           data-jenis="motor" onclick="selectJenis('motor')">
        <span class="jenis-btn-emoji">🏍️</span>
        <span class="jenis-btn-label">Motor</span>
      </div>
      <div class="jenis-btn {{ $currentJenis === 'truk' ? 'selected' : '' }}"
           data-jenis="truk" onclick="selectJenis('truk')">
        <span class="jenis-btn-emoji">🚛</span>
        <span class="jenis-btn-label">Truk</span>
      </div>
    </div>
  ...
  <button class="btn-batal {{ $currentPilihan ? 'show' : '' }}"
          id="btnBatal" onclick="batalkanPilihan()">
...
let selectedJenis   = @json($currentJenis);  // 'mobil' | 'motor' | 'truk'
let loading = false;
...
function updateSimpanBtn() {
  ...
  if (!selectedPilihan) { btn.disabled = true; return; }
...
function simpanPilihan() {
  ...
  if (selectedPilihan === 'kendaraan' && plat.length < 4) {
    showToast('Plat nomor tidak valid', 'error');
    return;
  }
